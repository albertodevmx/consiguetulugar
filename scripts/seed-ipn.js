...
/**
 * Script de automatización para poblar todo el contenido de IPN en Firestore.
 *
 * Reutiliza materias y temas que ya existan (ej: de UNAM) y crea solo los nuevos.
 *
 * Fases:
 ...
 *   5. Crear exámenes IPN y temas_config por área
 *
 * Uso:
 *   node scripts/seed-ipn.js
 *   node scripts/seed-ipn.js --skip-ai        # Solo crea estructura, no llama OpenAI
 *   node scripts/seed-ipn.js --only-lessons    # Solo genera lecciones faltantes
 *   node scripts/seed-ipn.js --only-questions  # Solo genera preguntas faltantes
 *
 * Auth (requerido para escritura en Firestore):
 *   FIREBASE_EMAIL=admin@email.com FIREBASE_PASSWORD=pass node scripts/seed-ipn.js
 *   (o el script te pedirá las credenciales interactivamente)
 *
 * El progreso se guarda en scripts/progreso-ipn.json para poder reiniciar.
 */
...
// ─── Paths ────────────────────────────────────────────────────────────────────
const DATA_PATH = path.join(__dirname, 'ipn-data.json');
const PROGRESS_PATH = path.join(__dirname, 'progreso-ipn.json');
...
async function phase1CreateMaterias(progress) {
  ...
  for (const materia of schoolData.materias) {
    ...
    // Exists in Firestore by name? (reuse from UNAM or other schools)
...
async function phase2CreateTemas(progress) {
  ...
  for (const materia of schoolData.materias) {
    ...
    for (const tema of materia.temas) {
      ...
      // Exists by name? (reuse from UNAM or other schools)
...
async function phase3GenerateLessons(progress, openaiConfig) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const promptText = `Crea una leccion educativa completa y bien estructurada sobre el tema "${tema.nombre_canonical}" (materia: ${materia.nombre_canonical}) para estudiantes que se preparan para el examen de admision al IPN (Instituto Politecnico Nacional) en Mexico.${subtemasText}
...
NO incluyas bloques de codigo markdown. Solo HTML puro.`;
    ...
    try {
      const res = await openaiWithRetry(apiKey, aiModel || 'gpt-4o-mini', [
        {
          role: 'system',
          content: 'Eres un profesor experto en preparacion para el examen de admision al IPN. Creas lecciones educativas claras, estructuradas y faciles de entender. Respondes solo con HTML valido.',
        },
...
async function phase4GenerateQuestions(progress, openaiConfig) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const promptText = `Genera exactamente ${needed} preguntas de opcion multiple para el examen de admision al IPN (Instituto Politecnico Nacional) sobre el tema "${tema.nombre_canonical}" (materia: ${materia.nombre_canonical}).${subtemasText}
...
IMPORTANTE: Solo el JSON array, sin markdown, sin texto extra, sin bloques de codigo.`;
    ...
    try {
      const res = await openaiWithRetry(apiKey, aiModel || 'gpt-4o-mini', [
        {
          role: 'system',
          content: 'Eres un experto en crear preguntas para el examen de admision al IPN. Genera preguntas precisas, variadas en dificultad, y con explicaciones educativas. Responde solo con JSON valido.',
        },
        { role: 'user', content: promptText },
      ]);
...
async function phase5CreateExamenes(progress) {
  console.log('\n══════════════════════════════════════════════════');
  console.log('  FASE 5: Crear Exámenes IPN y temas_config');
  console.log('══════════════════════════════════════════════════\n');

  const examenesCol = collection(db, 'examenes');

  // Check existing IPN exams
  const q = query(examenesCol, where('escuela', '==', 'IPN'));
  const existingSnap = await getDocs(q);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║   Seed IPN - Automatización de contenido         ║');
  console.log('╚══════════════════════════════════════════════════╝');
  ...
  console.log(`\nDatos: ${schoolData.materias.length} materias, ${totalTemas} temas, ${schoolData.areas.length} áreas`);
  ...
  if (!SKIP_AI) {
    ...
    try {
      ...
      console.error(`\n✗ ${err.message}`);
      console.log('  Puedes correr las fases 1-2 y 5 con: node scripts/seed-ipn.js --skip-ai');
      process.exit(1);
    }
  ...
  // Final summary
  console.log('\n╔══════════════════════════════════════════════════╗');
  console.log('║   ¡Proceso IPN completado!                       ║');
  console.log('╠══════════════════════════════════════════════════╣');
