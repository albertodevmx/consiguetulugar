...
/**
 * Script de automatización para poblar todo el contenido de UNAM en Firestore.
 *
 * Fases:
 ...
 *   5. Crear exámenes y temas_config por área
 *
 * Uso:
 *   node scripts/seed-unam.js
 *   node scripts/seed-unam.js --skip-ai        # Solo crea estructura, no llama OpenAI
...
// ─── Paths ────────────────────────────────────────────────────────────────────
const DATA_PATH = path.join(__dirname, 'unam-data.json');
const PROGRESS_PATH = path.join(__dirname, 'progreso.json');
...
// ─── Load data ────────────────────────────────────────────────────────────────
const unamData = JSON.parse(fs.readFileSync(DATA_PATH, 'utf-8'));
...
// ─── OpenAI API (raw HTTPS, same pattern as functions/index.js) ──────────────
...
async function phase1CreateMaterias(progress) {
  ...
  for (const materia of unamData.materias) {
    ...
    if (existingMap.has(materia.nombre_canonical)) {
      ...
      console.log(`  ✓ ${materia.nombre_canonical} (ya existe: ${existingId})`);
      skipped++;
      continue;
    }
  ...
  console.log(`\n  Resumen: ${created} creadas, ${skipped} existentes\n`);
}
...
async function phase2CreateTemas(progress) {
  ...
  for (const materia of unamData.materias) {
    ...
    for (const tema of materia.temas) {
      ...
      if (existingByName.has(tema.nombre_canonical)) {
        ...
        saveProgress(progress);
        console.log(`    ✓ ${tema.nombre_canonical} (ya existe: ${existingId})`);
        totalSkipped++;
        continue;
      }
  ...
  console.log(`\n  Resumen: ${totalCreated} creados, ${totalSkipped} existentes\n`);
}
...
async function phase3GenerateLessons(progress, openaiConfig) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const prompt = `Crea una leccion educativa completa y bien estructurada sobre el tema "${tema.nombre_canonical}" (materia: ${materia.nombre_canonical}) para estudiantes que se preparan para un examen de admision universitario en Mexico.${subtemasText}
...
async function phase4GenerateQuestions(progress, openaiConfig) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const prompt = `Genera exactamente ${needed} preguntas de opcion multiple para un examen de admision universitario sobre el tema "${tema.nombre_canonical}" (materia: ${materia.nombre_canonical}).${subtemasText}
...
IMPORTANTE: Solo el JSON array, sin markdown, sin texto extra, sin bloques de codigo.`;
    ...
    try {
      ...
      // Save questions in batches of 500
...
async function phase5CreateExamenes(progress) {
  console.log('\n══════════════════════════════════════════════════');
  console.log('  FASE 5: Crear Exámenes y temas_config');
  console.log('══════════════════════════════════════════════════\n');

  const examenesCol = collection(db, 'examenes');

  // Check existing UNAM exams
  const q = query(examenesCol, where('escuela', '==', 'UNAM'));
  const existingSnap = await getDocs(q);
  ...
  for (const areaConfig of unamData.areas) {
    console.log(`\n  📋 ${areaConfig.area}`);
    ...
    // Build the materia keys that belong to this area
    const materiaKeys = Object.keys(areaConfig.distribucion);

    for (const materiaKey of materiaKeys) {
      const materia = unamData.materias.find((m) => m.key === materiaKey);
      if (!materia) {
      ...
      // Distribute reactivos evenly among temas, with remainder going to first temas
...
async function main() {
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║   Seed UNAM - Automatización de contenido       ║');
  console.log('╚══════════════════════════════════════════════════╝');
  ...
  // Count total work
  let totalTemas = 0;
  for (const m of unamData.materias) totalTemas += m.temas.length;
  console.log(`\nDatos: ${unamData.materias.length} materias, ${totalTemas} temas, ${unamData.areas.length} áreas`);
  ...
  if (!SKIP_AI) {
    ...
    try {
      ...
      console.error(`\n✗ ${err.message}`);
      console.log('  Puedes correr las fases 1-2 y 5 con: node scripts/seed-unam.js --skip-ai');
      process.exit(1);
    }
  ...
  console.log('╠══════════════════════════════════════════════════╣');
  console.log(`║  Materias:     ${String(Object.keys(progress.materias).length).padStart(3)} / ${unamData.materias.length}                       ║`);
  ...
  console.log(`║  Exámenes:     ${String(Object.keys(progress.examenes).length).padStart(3)} / ${unamData.areas.length}                        ║`);
  console.log('╚══════════════════════════════════════════════════╝');
