export type { Materia } from './materia.model';
export type { Tema } from './tema.model';
export type { Subtema } from './subtema.model';
export type { Pregunta, OpcionPregunta, PreguntaStats } from './pregunta.model';
export type { Examen, MateriaMapping, TemaMapping } from './examen.model';
export type { Usuario, RolUsuario } from './usuario.model';

// Legacy models (used by admin components)
export type { School } from './school.model';
export type { ExamType } from './exam-type.model';
export type { Subject } from './subject.model';
export type { Unit } from './unit.model';
export type { Topic } from './topic.model';
export type { ExamTypeSubject } from './exam-type-subject.model';
export type { UnitTopic } from './unit-topic.model';
export type { Question } from './question.model';
